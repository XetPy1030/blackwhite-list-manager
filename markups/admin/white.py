...
class RemoveIpListCD(CallbackData, prefix='remove_ip_wl'):
    ip: str


class AddIpListCD(CallbackData, prefix='add_ip_wl'):
    pass


class PageListCD(CallbackData, prefix='page_wl'):
    page: int
...
def get_list_keyboard(ips: list[str], current_page: int = 0, total_pages: int = 1):
    ...
    kb.append(
        [
            types.InlineKeyboardButton(
                text='Добавить IP в белый список',
...
class AddListWhiteIPState(StatesGroup):
    ip = State()


AddListIPState = AddListWhiteIPState
